 ...
 * @author Jörg
 */
public class BlasterServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            if (outFormat.compareTo("plain")==0) {
                ...
                out.print(BlasterControl.getDimValue());
            }
                    ...
                    out.println ("<div class=\"measurement\">"+BlasterControl.getDimValue()+"</div>");
                ...
                out.println("<led>");
                out.println("<dim>"+BlasterControl.getDimValue()+"</dim>");
                out.println("</led>");
            }
            else if (outFormat.compareTo("json")==0) {
                response.setContentType("application/json;charset=UTF-8");
                out.println ("{ \"dim\": "+BlasterControl.getDimValue()+"}");
            }
            else {
                response.setContentType("text/plain;charset=UTF-8");
                out.print(BlasterControl.getDimValue());                
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("set")!=null) {
            ...
            if (set.compareTo("low")==0) {
             BlasterControl.turnOn(false);
            }
            if (set.compareTo("hight")==0) {
             BlasterControl.turnOn(true);
